...
//task running
void running(void **c_wait, void **c_run, void **c_fin, void **stiva, int time, int **cont_id, int *total_time, FILE *out)
{
...
void finish(void **c_wait, void **c_run, void **c_fin, void **stiva, int Q, int total_time, FILE *out, int **cont_id)
{
	...
	while (fgets(line, 256, in))
	{
		...
		if (strcmp(word, "run") == 0)
		{
			...
			if (time > Q)
			{
				while (time >= Q)
				{
					running(&c_wait, &c_run, &c_finish, &stiva, Q, &cont_ID, &total_time, out);
					time = time - Q;
				}
			}
			if (time >= 0)
			{
				running(&c_wait, &c_run, &c_finish, &stiva, time, &cont_ID, &total_time, out);
			}
		}
